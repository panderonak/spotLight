...
export default function CoverImageUpdater() {
  ...
  const handleCoverImageFileChange = (evt) => {
    const image = evt.target.files[0];
    setCoverImage(image);
    setCoverImagePreview(URL.createObjectURL(image));
  };

  // const currentUserCoverImage = useSelector((state) => state.auth.userInfo.coverImage);

  // setCoverImagePreview(currentUserCoverImage);
  ...
  const handleCoverImageUpdate = async () => {
    ...
    try {
      setLoading(true);
      const coverImageUpdateResponse = await userService.changeCover({
        coverImage,
      });
      if (coverImageUpdateResponse.success) {
        setMessage("Your cover image has been successfully updated.");
      }
    } catch (error) {
      ...
      <div className="w-full rounded-lg shadow-sm drop-shadow-md sm:w-1/2 lg:w-2/3">
        <div className="rounded-lg border">
          <div className="flex flex-wrap">
            <div
              className="relative my-12 mx-10 flex h-64 w-full rounded-3xl bg-slate-200 shadow-sm drop-shadow-md"
